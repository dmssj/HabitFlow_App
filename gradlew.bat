...
@rem Use java.home from local.properties or Android Studio bundled JBR
call :resolveLocalJava
if defined JAVA_HOME goto findJavaFromJavaHome
...
if exist "%APP_HOME%\local.properties" (
    for /f "usebackq eol=# tokens=1,* delims==" %%a in (`findstr /b "java.home=" "%APP_HOME%\local.properties"`) do (
        set "JAVA_HOME=%%b"
    )
)
